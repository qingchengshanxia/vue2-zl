...
Vue.prototype.$http = function (option) {
  ...
  if (option.url.indexOf('/member/login') == -1) {
    ...
    this.$axios.defaults.headers.common['Authorization'] = JSON.parse(localStorage.getItem('userInfo')).data.token;
  } else {
  ...
  this.$axios({ 
    ...
    switch (typeof obj.data=='string'&&Number(JSON.parse(obj.data).code)) {
      ...
      case 101:
        ...
        window.location.href = "https://potest.cofcotrading.com/";
        break;
      case 1:
        //请求成功，返回警告
        if (obj.data) {
