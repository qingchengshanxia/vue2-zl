<template>
  <el-container :class="{'change-big':boxchange,'hide-aside':hideAside}" :style="{height:pageHeight}">
    <el-header>
      ...
      <el-main :style="{height:containerHeight}">
        <div class="main-head-container">
          <div class="tag-div" v-show="isShowTagdiv">
            ...
            <div class="main-head" :style="{paddingLeft:paddingleft,paddingRight:paddingright,left:goleft,right:goright}">
              <div>
                <el-tag
                  ...
                  @click.native="goThisPage(tag,tag.id)"
                  >
                  <span>{{tag.menutitle}}</span>
                  <i @click.stop="handleClose(tag,index)" class="iconfont icon-guanbi"></i>
                </el-tag>
              </div>
            </div>
          ...
          <div class="tag-action" v-show="tags.length" :class="{'is-transparent':!isShowTagdiv}">
              <span>
                <el-dropdown>
                  <i class="iconfont icon-more_android_light" style="font-weight: bold;"></i>
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="closeOtherTag" v-show="isShowTagdiv"><span>关闭其它标签</span></el-dropdown-item>
                    <el-dropdown-item @click.native="hiddenTagDiv"><span>{{isShowTagdiv?'隐藏标签栏':'显示标签栏'}}</span></el-dropdown-item>
                    <el-dropdown-item @click.native="hiddenAsideDiv"><span>{{hideAside?'显示侧边菜单':'隐藏侧边菜单'}}</span></el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </span>
          </div>
...
<script>
  ...
  export default {
    ...
    methods:{
      setCurMenuType(type){
        // aside组件中，切换一级菜单的函数
        this.$refs.aside.switchMainMenu(type);
      },
      ...
      goThisPage(tag,id){
        ...
        */
        ...
        this.$refs.aside.switchTargetMenu(tag, id); //aside组件方法。切换一级菜单，然后定位到目标二级菜单
    ...
    mounted(){
      ...
      if(localStorage.getItem('tagmenu')&&localStorage.getItem('activetagmenu')){
        ...
        this.$refs.aside.changeTitle(activetagmenu.menutitle,activetagmenu.url,activetagmenu.id,activetagmenu.menutype);
      }else{
...
<style type="text/css" lang="scss">
    ...
    .main-head-container{
      .el-tag {
        ...
        &.active {
          span {
            border-bottom:solid 1px #214c86;
            color: #214c86;
          }
        }
        >span {
